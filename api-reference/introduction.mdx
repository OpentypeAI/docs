...
| `ceiling_micros`, the per-run spend ceiling | `5000` | $0.005 |
| `balance_micros` after the sign-up credit | `5000000` | $5.00 |
| `amount_micros` for a checkout | `25000000` | $25.00 |

`balance_micros` and billing transaction amounts are signed; every other amount is zero or positive. Neon 1.1 costs $0.042 per million tokens for input and for output, and each part is rounded up to a whole micro-USD separately. A run holds its whole spend ceiling, at most 5,000 micro-USD, against your credit and quota until it settles. See [Models and pricing](/getting-started/models-and-pricing).
...
| Input estimate per run | 64,000 tokens; 3,968 tokens for a Neon 1.1 decision |
| Spend per run | 5,000 micro-USD |
| `deadline_ms` | 1 to 120,000, default 30,000 |
