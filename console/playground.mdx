...
| **Not enough credit** | 402 `insufficient_credits` | The balance does not cover the read. A read holds up to $0.02 while it runs, so it needs at least that much to start | Choose the **Billing** link in the panel and add funds, or turn on auto-recharge. See [insufficient_credits](/problems/insufficient_credits) |
