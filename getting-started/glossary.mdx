...
| Term | What it means |
| --- | --- |
| **hold** | The credit a run reserves when it is admitted: its whole per-request ceiling of 5,000 micro-USD ($0.005), kept until the run settles and then replaced by the actual cost. Holds are subtracted from `balance_micros`, so a run needs at least 5,000 micro-USD of available credit to start, or it gets `402 insufficient_credits`. Usage reports show held amounts as `reserved_micros`. A run that fails before a model served it has its hold released. See [Handling insufficient credits](/guides/handling-insufficient-credits). |
