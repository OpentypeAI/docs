...
Runs are paid from prepaid credit. Before a run starts, OpenType holds the run's full per-request spend ceiling, at most 5,000 micro-USD ($0.005), against your balance until the run settles. A run that would take the available balance below zero is refused.

In practice, a run needs at least 5,000 micros of available credit to start, even though a typical decision run costs far less. Holds of runs still in flight count against the balance until they settle.
...
4. Retry the run once `balance_micros` is at least 5,000. The first attempt was refused before a run existed, so reuse the same `Idempotency-Key`.
