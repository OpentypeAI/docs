---
title: "provider_unauthorized"
description: "HTTP 503 on POST /v1/runs: the model service refused OpenType's own credentials. Not your API key. Retry later with a new Idempotency-Key."
icon: "user-lock"
keywords: ["provider_unauthorized", "503", "provider", "model service", "POST /v1/runs"]
---
...
| HTTP | `code` | Retryable |
| --- | --- | --- |
| `503` | `provider_unauthorized` | Later, with a new `Idempotency-Key`. Retrying immediately does not help. |

## What happened

Route: `POST /v1/runs`.

The model service that runs Neon 1.1 rejected the credentials OpenType uses to call it. This is a service-side configuration problem. Your own key was accepted: a problem with it answers `401 invalid_credential` before any run starts.
...
```json
{"error":{"code":"provider_unauthorized","message":"the provider call failed","request_id":"req_7d3f0c1a9b2e4f6a8c0d1e2f3a4b5c6d"}}
```
