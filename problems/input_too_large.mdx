...
icon: "text-width"
keywords: ["input_too_large", "413", "tokens", "context", "3,968 tokens", "64,000 tokens", "input estimate", "POST /v1/runs"]
---
...
| --- | --- | --- | --- |
| Admission | the estimate is over 64,000 tokens | refused before a run existed; nothing stored or charged | the same `Idempotency-Key` |
| Routing | the input does not fit the model's context; for Neon 1.1 decision runs, 3,968 tokens | the run was admitted, then failed before any model call; nothing charged, and the run stays `pending` | a **new** `Idempotency-Key` |

For Neon 1.1, the routing limit is the one you meet first: a decision run's input must fit in 3,968 tokens, far below the 64,000-token ceiling.

The admission message names the estimate, as in the example below. Branch on `code`, not on the message.

## How to fix

1. Estimate before you send: count the UTF-8 bytes of the prompt and of the contract, divide each by 4 and round up. Keep a decision run's total well under 3,968 tokens.
...
4. Retry with the right key:
   - If the input was refused at admission (over 64,000 tokens), the key is still free; reuse it.
...
echo "estimated input: $EST tokens (decision limit 3968)"
```

```typescript TypeScript
const DECISION_CONTEXT_TOKENS = 3968;
...
- [body_too_large](/problems/body_too_large) - the body is over 1 MiB before any token count.
