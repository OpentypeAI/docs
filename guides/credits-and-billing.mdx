...
- **Your balance is credited minus spent minus held.** Purchases and grants add to it, usage takes from it, and every run in flight holds its spend ceiling, at most 5,000 micros ($0.005), until it settles.
- **A run needs 5,000 micros of available credit to start.** It holds that whole ceiling while it runs, then settles at its real cost, which is usually far lower. If the balance after the hold would go below zero, the run is refused with [`402 insufficient_credits`](/guides/handling-insufficient-credits).
...
- **The hold is not a charge.** The 5,000 micros held while a run is in flight return to the balance when it settles; only the settled cost is spent.
