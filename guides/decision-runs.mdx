...
res = requests.post(
    ...
    data=body,
    timeout=130,  # longer than the largest deadline_ms (120 s)
)
run = res.json()
if not res.ok:
...
| `deadline_ms` | integer | no | `30000` | Clamped to 1 to 120,000: `0` becomes `1`, and anything above 120,000 becomes 120,000. |
...
The whole request body is capped at 1 MiB (`413 body_too_large`).
...
- A run may never cost more than 5,000 micro-USD ($0.005). That whole ceiling is held from your balance while the run is in flight, and the run is charged its real cost when it settles.
...
### Stay under 3,968 input tokens

Neon 1.1 reads at most 3,968 input tokens per decision. The input estimate is roughly a quarter of the byte length of the `state`, plus a quarter of the byte length of `instructions` and `questions`. A run above the limit is refused with `413 input_too_large`.
...
assert input_estimate(json.load(open("triage.json"))) < 3968
```

Trim long states to the fields the questions actually need. Because the body changes when you trim, send it with a new `Idempotency-Key`.

### Set a client timeout above the deadline

`deadline_ms` defaults to 30 seconds and never exceeds 120 seconds. Give your HTTP client a longer timeout than the deadline you send, so the server, not your client, decides when the run is over. A run that runs out of time returns `504 deadline_exceeded`.
...
| `413` | [`input_too_large`](/problems/input_too_large) | The input estimate is above 3,968 tokens | Shrink the `state` or the questions, then retry with a new `Idempotency-Key`. |
...
- [Playground](/console/playground) - build and try a question set in the console before you code it.
