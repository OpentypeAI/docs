...
- [ ] **`deadline_ms` is set on purpose.** It defaults to 30,000 and is clamped to 1 to 120,000. A run that misses it fails with `504 deadline_exceeded`.
- [ ] **Your HTTP client timeout is longer than `deadline_ms`.** `POST /v1/runs` answers only when the run has settled. Allow at least 10 seconds of margin over `deadline_ms`.
...
- [ ] **Decision inputs stay under 3,968 tokens.** The estimate is the bytes of `state` divided by 4, rounded up, plus the bytes of the question set (`instructions`, `questions`, `draws` and `think_tokens`) divided by 4, rounded up. Above 3,968, the run is refused with `413 input_too_large`. See [Models and pricing](/getting-started/models-and-pricing).
- [ ] **Bodies stay under 1 MiB.** Larger bodies are refused with `413 body_too_large`.
...
- [ ] **Auto-recharge is on**, with a card saved, a threshold above one hour of spend plus 5,000 micros, and an amount above your busiest hour. It charges at most once per organization per clock hour. See [Auto-recharge](/guides/auto-recharge).
- [ ] **The balance covers your concurrency.** Every run in flight holds its spend ceiling, up to 5,000 micros ($0.005), until it settles.
